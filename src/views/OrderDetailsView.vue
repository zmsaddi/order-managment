...
// استيراد دالة إنشاء الفاتورة من ملف utils/invoiceGenerator.js
...
export default {
  ...
  setup() {
    ...
    // تحديث عنوان الصفحة عند تحميل المكون
    ...
    const fetchOrder = async () => {
      try {
        ...
        // تحديث عنوان الصفحة
        emit('update-page-title', `تفاصيل الطلب #${orderId}`)
...
الحالة: ${getStatusText(order.value.status)}`
    ...
    const generateInvoice = () => {
      ...
      try {
        // استدعاء دالة إنشاء الفاتورة من ملف utils/invoiceGenerator.js
        generateInvoice(order.value, salesRep.value)
      } catch (error) {
...
/* تنسيقات إضافية */
.btn {
  @apply px-4 py-2 rounded-md text-sm font-medium focus:outline-none transition-colors;
}
