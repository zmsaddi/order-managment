...
import rawNotoArabic from '@/assets/fonts/NotoSansArabic-Regular.ttf?base64';

// Strip off the data:…;base64, prefix before giving it to jsPDF
const base64NotoArabic = rawNotoArabic.includes(',') ? rawNotoArabic.split(',')[1] : rawNotoArabic;

// Register the font once, before creating any new jsPDF instance
jsPDF.API.events.push(['addFonts', function() {
  try {
    // pass in pure Base64 (no data:… prefix) to addFileToVFS
    this.addFileToVFS('NotoSansArabic-Regular.ttf', base64NotoArabic);
    this.addFont('NotoSansArabic-Regular.ttf', 'NotoSansArabic', 'normal');
  } catch (error) {
    console.warn('Failed to load Arabic font, falling back to helvetica:', error);
  }
}]);
...
export const generateInvoice = (order, products, salesRep) => {
  ...
  // إعداد الخط العربي مع fallback
  try {
    doc.setFont('NotoSansArabic');
  } catch (error) {
    console.warn('Arabic font not available, using helvetica');
